...
// ParsePKCS8PrivateKey parses a private key in PKCS#8 format.
// It falls through to Go's x509.ParsePKCS8PrivateKey for all key types.
...
func ParsePKIXPublicKey(data []byte) (crypto.PublicKey, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "error parsing PKIX public key")
	}
	return pub, nil
}
